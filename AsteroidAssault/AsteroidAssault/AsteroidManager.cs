...
namespace AsteroidAssault
{
    class AsteroidManager
    {
        ...
        {
            Sprite newAsteriod = new Sprite(new Vector2(-500, -500),
                texture, initialFrame, Vector2.Zero);
            for (int x = 1; x < asteroidFrames; x++)
            {
                newAsteriod.AddFrame(new Rectangle(initialFrame.X +
                    (initialFrame.Width * x),
                    initialFrame.Y, initialFrame.Width,
                    initialFrame.Height));
            }
            newAsteriod.Rotaton = MathHelper.ToRadians((float)rand.Next(0, 360));
            newAsteriod.CollisionRadius = 15;
            Asteroids.Add(newAsteriod);
        }
        ...
        private Vector2 randomLocation()
        {
            ...
            locationOK = true;
                ...
                {
                if(asteroid.IsBoxColliding(new Rectangle((int)location.X, (int)location.Y,
        ...
        {
            if (asteroid.Destination.Intersects(new Rectangle(-screenPadding, -screenPadding,
